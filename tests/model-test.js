#!/usr/bin/env node
// node tests/model-test.js -- KeyboardLayoutModel.js, with the assertions of Omarchy's own
// test/shell.d/keyboard-layout-test.sh run against this plugin's copy of the model.
"use strict"
const path = require("path")
const assert = require("assert")

const model = require(path.resolve(__dirname, "..", "KeyboardLayoutModel.js"))
...
// The helper's `layouts` output is a filtered listing: block starts collapsed to
// "- " and only brief/description lines kept. It must read the same.
const filtered = [
  "- ",
  "  description: Generic 105-key PC",
  "- ",
...
assertEqual(JSON.stringify(model.layoutBriefs(filtered)), JSON.stringify(briefs), "the helper's filtered listing reads the same table")
...
if (failures.length) {
  console.error(failures.map(f => "not ok - " + f).join("\n"))
  console.error(`${failures.length} failed, ${passed} passed`)
  process.exit(1)
}
console.log(`ok - ${passed} model assertions`)
